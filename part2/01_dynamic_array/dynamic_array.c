...
{
    if(initial_capcity==0)  initial_capcity = INITIAL_CAPACITY;
...
{
    if(index>arr->size)    return -1;   //可以在末尾插入，因此是大于size
...
int array_delete(DynamicArray* arr,size_t index)
{
    ...
    if(arr->size>0&&arr->size<=arr->capacity/4&&arr->capacity>INITIAL_CAPACITY)
    {
        ...
        if(new_capacity<arr->size)  new_capacity=INITIAL_CAPACITY;
